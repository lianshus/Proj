/*
 * @Author: diana
 * @Date: 2023-06-02 13:28:56
 * @LastEditTime: 2023-06-05 18:53:45
 */
import React, { useState, useEffect, useContext } from 'react';
import { Space, Switch, Table, Button, Tag, Modal, Input } from 'antd';
import { MyContext } from '../../interact';
import { ethers } from 'ethers';

import EchartsCircle from '../../components/AdminInfo/Detail';
import { useSelector } from 'react-redux';

import LineChart from '../../components/AdminInfo/LineChart';


export default function IProvider() {
    ...
    const [isUntrust, setIsUntrust] = useState();
    ...
    const handleButtonShow = async (rowData) => {
        ...
        whichType(rowData.address);
        isUntrustworthy(rowData.address);
    ...
    // 点击行内按钮的回调函数
    const handleButtonAddDeprave = (rowData) => {
        addBlackList(rowData.address);
    }

    // 核算
    const handleButtonAccounting = (rowData) => {
        setSelectedRowData(rowData);
        setIsModalOpen(true);
    }
    // 输入值改变，获取数据
        ...
        },
        {
            ...
            render: (_, record) => (
                <Space size="middle">
                    ...
                    <Button onClick={() => handleButtonAccounting(record)} >Accounting</Button>
                    <Button onClick={() => handleButtonAddDeprave(record)} danger>AddDeprave</Button>
                </Space>
            ),
        },
    ];
    ...
    const getUser = async () => {
        try {
            // 获取
            const DATAU = await contracts[2].getAllMem();
            const DATAC = await contracts[3].getAllMem();
            const DATA = [...DATAU, ...DATAC];
            // 组装成需要的
    ...
    // 判断成员类型
    const whichType = async (account) => {
        try {
            const user = await contracts[2].isMember(account);
            if (user) {
                setType(1)
            } else {
                setType(2)
    ...
    const showDetails = async (account) => {
        // 判断成员类别，公司还是个人
        const type = await contracts[2].isMember(account);
        try {
            // 通过类别调用不同合约方法
            if (type) {
                ...
                const DATA = await contracts[7].findCompany(account);
                // 组装成需要的
                const det = {
                    address: DATA.addr,
                    qutoa: DATA.qutoa.toString(),
    ...
    // 查看用户是否欠款
    const isUntrustworthy = async (account) => {
        try {
            const tx = await contracts[4].isUntrustworthy(
                account,
                { gasLimit: 500000 });
            setIsUntrust(tx)
        } catch (e) {
    ...
    // 获取黑名单功能
    const getBlackList = async (account) => {
        try {
            const DATA = await contracts[4].getDeprave()
            const users = DATA.map((item, index) => {
                return {
                    order: index + 1,
                    address: item,
    ...
    // 3. 加入黑名单
    const addBlackList = async (account) => {
        try {
            const tx = await contracts[4].addDeprave(
                account,
                { gasLimit: 500000 });
            tx.wait();
            console.log(tx)
    ...
    // 2. 核算
    const recordEmssion = async () => {
        try {
            console.log(selectedRowData.address)
            const tx = await contracts[4].recordEmission(
                // 地址
    ...
    return (
        <>  
        <div className='flex flex-row items-center justify-center w-full h-64 mt-5'>
            <div className='flex items-center justify-center w-4/5 h-60 py-2 px-2 rounded-lg mt-2 mb-5 border' >
                    <LineChart /> 
            </div>
        </div>
            {/* <button onClick={getProofL}></button> */}
            <div className='flex flex-col items-center justify-center w-full min-h-screen py-2 px-2  rounded-lg mt-2 mb-5 '>
                {/* 详情 */}
                <div className='w-4/5 rounded-lg h-72 mt-2 py-2'>
                    {
                        // 默认点击第一个数据
                        detail ? 
                        (<>
                            <div className='flex flex-row '>
                                <div className='w-1/2 border rounded-lg h-64  bg-white '>
                                    {/* 信息 */}
                                ...
                                <div className='w-1/2 h-64 flex-col ml-5'>
                                    ...
                                    <div className='h-15 flex flex-row border bg-white  rounded-lg'>
                                        ...
                                        <div className='mt-2 ml-20 flex flex-row'>
                                            <p className='text-blue-500  text-lg mt-1 ml-10'>
                                                {type === 1 ? 'Personal User' : 'Company User'}
                                            </p>
                                        </div>
                                    ...
                                    <div className='h-48 flex flex-col items-center justify-center bg-white border rounded-lg mt-4'>
                                        <div>This year the client </div>
                                        <br></br>
                                        {isUntrust ?
                                            (<div className='text-adminbg'>Be in Debt
                                            </div>) :
                                            (<div className='text-adminbg'>Good Reputation
                                            </div>)}
                                    </div>
                                </div>
                            </div>
                    ...
                    <Modal open={isModalOpen} onCancel={handleCancel} footer={null}>
                        ...
                        >
                            <h1 className="text-start text-2xl font-bold text-indigo-600 sm:text-3xl mb-10">
                                Record how many this user emssion
                            </h1>
                ...
                {/* 表格 */}
                <div className='w-4/5 border border-gray-300 rounded-lg flex flex-row min-h-screen bg-white'>
                ...
                {/* 黑名单数据区域 */}
                <div className='w-4/5 border border-gray-300 rounded-lg flex flex-row h-128 bg-white mt-5'>
                    <div className='w-full h-full'>
                        <div className='px-2'>
                            <Table columns={columnsBlackList} dataSource={blacklist} />
                        </div>
                    </div>
                </div>
            </div>
